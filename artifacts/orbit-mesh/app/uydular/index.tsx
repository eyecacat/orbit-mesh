// app/uydular/index.tsx — Türk Uyduları (doğru NORAD ID + yedek TLE)
...
import { computePasses, fetchTleByCatnr, fetchTlesFromGroup, getPosition, GeoPos, PassInfo, TleData } from "@/services/satelliteTracker";

const FALLBACK = { lat: 39.0, lon: 35.0 };
// DÜZELTME: eski ID'ler yanlıştı (39084=Landsat 8!). Doğruları CelesTrak'ten doğrulandı.
const LEO_SATS = [
  { key: "G1A", short: "GÖKTÜRK-1A", catnr: 41875, desc: "Yüksek çözünürlüklü gözlem uydusu" },
  { key: "G2", short: "GÖKTÜRK-2", catnr: 39030, desc: "Gözlem uydusu" },
  { key: "IMECE", short: "İMECE", catnr: 56178, desc: "Millî gözlem uydusu (TÜBİTAK)" },
...
interface SatEntry { key: string; short: string; fullName: string; desc: string; tle: TleData | null; pos: GeoPos | null; passes: PassInfo[]; isGeo: boolean; fromFallback: boolean; }
...
export default function UydularScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      let loc = { ...FALLBACK }; let real = false;
      if (status === "granted") {
        ...
        if (known) { loc = { lat: known.coords.latitude, lon: known.coords.longitude }; real = true; }
      }
      setObs(loc); setObsReal(real);
      ...
      const leoEntries: SatEntry[] = (await Promise.all(
        LEO_SATS.map(async (s) => {
          ...
          if (!tle) return null;
          return {
            key: s.key, short: s.short, fullName: tle.name, desc: s.desc, tle,
            pos: getPosition(tle),
            passes: computePasses(tle, loc.lat, loc.lon, { hours: 72, minEl: 10 }),
            isGeo: false, fromFallback: !!tle.fromFallback,
          } as SatEntry;
        })
      )).filter((x): x is SatEntry => x !== null);

      const geoEntries: SatEntry[] = geoTles.map((t) => {
        const m = t.name.match(/TURKSAT\s*([0-9A-C]+)/i);
        return {
          key: m?.[1] ?? t.name, short: m?.[1] ? `T${m[1]}` : t.name, fullName: t.name,
          desc: "Sabit yörünge (GEO)", tle: t, pos: getPosition(t), passes: [],
          isGeo: true, fromFallback: !!t.fromFallback,
        };
      });

      const all = [...geoEntries, ...leoEntries];
      setSats(all);
    ...
    <View style={[styles.root, { backgroundColor: colors.background, paddingTop: topPad }]}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 60 }}
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chips}>
          {sats.map((s) => (
            <Pressable key={s.key} onPress={() => setSelected(s.key)}
              style={[styles.chip, { backgroundColor: selected === s.key ? colors.danger + "22" : colors.card, borderColor: selected === s.key ? colors.danger : colors.border }]}>
            ...
            <View style={styles.satHeader}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={[styles.muted, { color: colors.mutedForeground }]}>
                  {current.desc} · {current.fromFallback ? "yerleşik TLE (yedek)" : "TLE: CelesTrak (canlı)"}
                </Text>
              </View>
            </View>
            ...
            <View style={styles.block}>
              ...
              {current.pos ? (
                <Text style={[styles.blockValue, { color: colors.primary }]}>
                  {current.pos.lat.toFixed(2)}°, {current.pos.lon.toFixed(2)}° · {Math.round(current.pos.altKm)} km{current.isGeo ? " (GEO)" : ""}
                </Text>
              ) : (
              ...
              {current.isGeo && (
                <Text style={[styles.mutedSmall, { color: colors.mutedForeground }]}>
                  Sabit yörünge uydusu — Türkiye'den her zaman görünür.
                </Text>
              )}
            </View>
                ...
                {current.passes.length === 0 ? (
                  ...
                  current.passes.slice(0, 3).map((p, i) => (
                    <Text key={i} style={[styles.passLine, { color: colors.foreground }]}>
                      ...
                      {" "}· {p.maxEl.toFixed(0)}° yükseliş · {((p.endMs - p.startMs) / 60000).toFixed(0)} dk
                    </Text>
                  ))
                )}
              </View>
