...
const PRESETS: { id: LabPreset; label: string; metrics: Metrics }[] = [
  { id: "earth", label: "Dünya-benzeri", metrics: { temperature: 15, water: 80, atmosphere: 75, gravity: 100, radiation: 15 } },
  { id: "mars", label: "Mars-benzeri", metrics: { temperature: -63, water: 20, atmosphere: 12, gravity: 38, radiation: 78 } },
  { id: "venus", label: "Venüs-benzeri", metrics: { temperature: 465, water: 5, atmosphere: 98, gravity: 90, radiation: 55 } },
  { id: "ice", label: "Buz-Dünya", metrics: { temperature: -120, water: 55, atmosphere: 35, gravity: 80, radiation: 25 } },
];
...
function verdict(score: number) {
  if (score >= 75) {
    return {
      ...
      detail: "Koşullar Dünya'ya yakın. Sıvı su ve dengeli atmosfer olasılığı yüksek.",
      color: "#22C55E",
    };
  }
  if (score >= 45) {
    ...
    <View style={[styles.metricCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
      <View style={styles.metricTopRow}>
        <View>
          ...
          <Text style={[styles.metricDesc, { color: colors.mutedForeground }]}>{description}</Text>
        </View>
      ...
      <View style={styles.rangeRow}>
        <Text style={[styles.rangeText, { color: colors.mutedForeground }]}>{min}</Text>
        <Text style={[styles.rangeText, { color: colors.mutedForeground }]}>{max}</Text>
      </View>
    </View>
  );
}

export default function YasabilirlikLabScreen() {
  const colors = useColors();
  ...
  function applyPreset(preset: LabPreset) {
    ...
    setMetrics(PRESETS.find(p => p.id === preset)?.metrics ?? metrics);
  }
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={{ paddingTop: topPad + 12, paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={[styles.kicker, { color: colors.mutedForeground }]}>Yaşanabilirlik Laboratuvarı</Text>
            <Text style={[styles.title, { color: colors.foreground }]}>Bir gezegeni yaşanabilir hale getir</Text>
          </View>
        </View>

        <View style={[styles.hero, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <LinearGradient colors={["#0EA5E9", "#7C3AED", "#0F172A"]} style={styles.heroGradient}>
        ...
        <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Hızlı senaryolar</Text>
        ...
        <MetricControl
          ...
          onChange={temperature => setMetrics(prev => ({ ...prev, temperature }))}
        />
        ...
        <MetricControl
          ...
          onChange={atmosphere => setMetrics(prev => ({ ...prev, atmosphere }))}
        />
        ...
        <MetricControl
          ...
          onChange={radiation => setMetrics(prev => ({ ...prev, radiation }))}
        />

        <View style={[styles.summaryCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <Text style={[styles.sectionTitle, { color: colors.foreground, paddingHorizontal: 0 }]}>Sonuç özeti</Text>

          <View style={styles.summaryRow}>
            <View style={[styles.summaryBadge, { backgroundColor: currentVerdict.color + "18" }]}>
              <Feather name="check-circle" size={14} color={currentVerdict.color} />
              <Text style={[styles.summaryBadgeText, { color: currentVerdict.color }]}>
                {currentVerdict.label}
              </Text>
            </View>
            <Text style={[styles.summaryScore, { color: colors.foreground }]}>{score}/100</Text>
          </View>

          <Text style={[styles.summaryText, { color: colors.mutedForeground }]}>
            Sıcaklık, su, atmosfer, yerçekimi ve radyasyon birlikte değerlendirildi.
            Bu ekran, gezegen koşullarını değiştirerek yaşam olasılığını öğretici biçimde gösterir.
          </Text>

          <View style={styles.reasonList}>
            <View style={styles.reasonRow}>
              <Feather name="thermometer" size={14} color={colors.primary} />
              <Text style={[styles.reasonText, { color: colors.foreground }]}>Sıcaklık: {metrics.temperature}°C</Text>
            </View>
            <View style={styles.reasonRow}>
              <Feather name="droplet" size={14} color={colors.primary} />
              <Text style={[styles.reasonText, { color: colors.foreground }]}>Su: {metrics.water}%</Text>
            </View>
            <View style={styles.reasonRow}>
              <Feather name="cloud" size={14} color={colors.primary} />
              <Text style={[styles.reasonText, { color: colors.foreground }]}>Atmosfer: {metrics.atmosphere}%</Text>
            </View>
            <View style={styles.reasonRow}>
              <Feather name="compass" size={14} color={colors.primary} />
              <Text style={[styles.reasonText, { color: colors.foreground }]}>Yerçekimi: {metrics.gravity}%</Text>
            </View>
            <View style={styles.reasonRow}>
              <Feather name="zap" size={14} color={colors.primary} />
              <Text style={[styles.reasonText, { color: colors.foreground }]}>Radyasyon: {metrics.radiation}%</Text>
            </View>
          </View>
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  metricDesc: { fontSize: 12, lineHeight: 18, fontFamily: "Inter_400Regular", marginTop: 2 },
  ...
  summaryText: { fontSize: 13, lineHeight: 20, fontFamily: "Inter_400Regular" },
