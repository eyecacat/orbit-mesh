...
export function useDemoMode(enabled: boolean): DemoTelemetry {
  const [telemetry, setTelemetry] = useState<NodeTelemetry>({
    nodeId: "ORBIT-DEMO", timestamp: Date.now(), vlf_hz: 7.83, vlf_amplitude: 820,
    battery: 87, temp_c: 24.5, gx: 0.12, gy: -0.05, gz: 0.88,
    ax: 0.01, ay: 0.02, az: 0.98, anomaly: false, receivedAt: Date.now(),
  });
  const [anomalyScore, setAnomalyScore] = useState<{ total: number; level: string } | null>(null);

  const generateFrame = useCallback(() => {
    const now = Date.now(); const t = now * 0.001;
    const baseFreq = 7.83;
    const noise = (Math.random() - 0.5) * 0.4;
    const burst = Math.sin(t * 0.3) > 0.92 ? (Math.random() - 0.5) * 3 : 0;
    const vlf = Math.max(0, baseFreq + noise + burst);
    const baseAmp = 800 + Math.sin(t * 0.5) * 100;
    const ampBurst = burst !== 0 ? Math.random() * 400 : 0;
    const amp = Math.max(0, baseAmp + (Math.random() - 0.5) * 60 + ampBurst);
    const isAnomaly = burst !== 0 && Math.abs(burst) > 1.2;

    setTelemetry({
      nodeId: "ORBIT-DEMO", timestamp: now, vlf_hz: vlf, vlf_amplitude: amp,
      battery: 87, temp_c: 24.5 + Math.sin(t * 0.1) * 1.5,
      gx: 0.12 + (Math.random() - 0.5) * 0.1, gy: -0.05 + (Math.random() - 0.5) * 0.1, gz: 0.88 + (Math.random() - 0.5) * 0.1,
      ax: 0.01 + (Math.random() - 0.5) * 0.02, ay: 0.02 + (Math.random() - 0.5) * 0.02, az: 0.98 + (Math.random() - 0.5) * 0.02,
      anomaly: isAnomaly, receivedAt: now,
    });
