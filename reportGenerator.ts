// ============================================================
// ORBIT-MESH — Bilimsel Gözlem Raporu Üretici
// ------------------------------------------------------------
// AsyncStorage'da biriken gözlem kayıtlarını (orbit_mesh_observations)
// PDF ve CSV formatında dışa aktarır. "Ölçtük" yerine "bilimsel gözlem
// oluşturduk" hissi vermek için her gözleme bir Observation ID,
// zaman damgası, sinyal/FFT/trend/confidence/güvenilirlik bilgisi
// ekler.
// ============================================================

export interface ObservationRecord {
  id: string; // Observation ID, örn: OBS-20260630-0001
  ...
  confidence_score?: number; // bizim hesapladığımız Sensör Sağlık genel skoru
...
// ── PDF (HTML şablonu, expo-print ile PDF'e çevrilir) ───────────────────
...
export function buildReportHtml(records: ObservationRecord[], meta: { school?: string; city?: string }): string {
  ...
  const avgConfidence =
    total > 0 ? Math.round(records.reduce((s, r) => s + (r.confidence_score ?? 0), 0) / total) : 0;
  ...
  const rowsHtml = records
    ...
    .map(
      ...
      <tr>
        ...
        <td>${r.space_state ?? "—"}</td>
        <td>${r.trend ?? "—"}</td>
        <td style="color:${qualityColor(r.confidence_score ?? 0)}; font-weight:bold;">
          %${r.confidence_score ?? "—"}
        </td>
...
<meta charset="utf-8" />
<style>
  body { font-family: -apple-system, Arial, sans-serif; color: #0f172a; padding: 24px; }
...
<body>
  ...
  <div class="subtitle">
    ${meta.school ?? "ORBIT School"} ${meta.city ? "· " + meta.city : ""} · Oluşturulma: ${generatedAt}
  </div>

  <div class="summary">
    <div class="summary-box">
      <div class="label">Toplam Gözlem</div>
      <div class="value">${total}</div>
    </div>
    <div class="summary-box">
      <div class="label">Ortalama Güven</div>
      <div class="value" style="color:${qualityColor(avgConfidence)}">%${avgConfidence}</div>
    </div>
    <div class="summary-box">
      <div class="label">Anomali Sayısı</div>
  ...
  <div class="footer">
    ORBIT-MESH — Türkiye Okul Bilim Gözlem Ağı · TEKNOFEST İnsanlık Yararına Teknolojiler Yarışması
  </div>
</body>
</html>`;
}